            ...
            if (attrs) {
                attrs.forEach(function (attr) {
                    if ((attr.key.value == 'name' || !attr.value) && f) {
                        ret += '\n__func["__fn__' + attr.value.value + '"]({';
                        f = false;
                    }
                    else {
        ...
        _getAppend: function (node) {
            ...
            if (attrs) {
                attrs.forEach(function (attr, index) {
                    if ((attr.key.value == 'var' || (!attr.value && index == 0))) {
                        var varaNode = attr.value ? attr.value : attr.key;
                        if (varaNode.type == 'STR') {
                            vara = me._getStr(varaNode);
                        }
                    }
                    if ((attr.key.value == 'value' || (!attr.value && index == 1))) {
                        value = me._getExpr(attr.value ? attr.value : attr.key);
                    }
                    if (attr.key.value == 'index') {
                        ind = '"' + attr.value.value + '"';
                    }
                });
                if (vara && value) {
