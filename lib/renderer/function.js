...
module.exports = function (Renderer) {
    ...
    utils.mixin(Renderer.prototype, {
        ...
        _getFunction: function (node, f) {
            ...
            var name = node.name;
            var ret;
            var fobj = me.engine.func;
            var mobj = { 'strip': 1 };
            if (fobj[name] && !mobj[name]) {
                if (f === true) {
        ...
        _getFunc: function (node) {
            ...
            ret += 'for(var _ in __p){if(__p.hasOwnProperty(_)&&__da[_]==undefined){__da[_]=__p[_];}}\n'
                + 'var ' + func + ' = __p;'
        ...
        _callFunc: function (node, flag) {
            ...
            if (funcName && attrs) {
                if (attrs.length == 0) {
                    ...
                    return '__func["__fn__' + funcName + '"](' + params +')' + (flag == 2 ? '' : ';');
                }
            }
            else {
        ...
        _getCapture: function (node) {
            ...
            if (attrs) {
                attrs.forEach(function (attr) {
                    ...
                    var valObj = attr.value;
                    if (key == 'name' || !valObj) {
                        // todo: $smarty.capture.name
                    }
                    if (key == 'assign' && valObj.type == 'STR') {
                        me.capAssign[valObj.value] = true;
                        ret += 'function __cap__' + valObj.value + '(){' + me._init(block) + '}';
                    }
                    if (key == 'append' && valObj.type == 'STR') {
                        var f = me.capAssign[valObj.value];
                        if (f) {
                            ret += '__da.' + valObj.value + '["' + f + '"] = function(){var __ca="";' 
                                + me._init(block).replace(/__h/g, '__ca') 
                                + 'return __ca;}';
                            me.capAssign[valObj.value]++;
                        }
                        else {
                            ret += '__da.' + valObj.value + '={"0": function(){var __ca="";' 
                                + me._init(block).replace(/__h/g, '__ca') 
                                + 'return __ca;}};';
                            me.capAssign[valObj.value] = 1;
                        }
                    }
                });
            }
