...
module.exports = {escape: function (str, f) {
    var tmp = '';
    function padnum (n, w, r, p) {
        n = n.toString(r || 10); p = p || "0"; while (n.length < w) {n = p + n;} return n.toUpperCase();
    }
    if (!str) return '';
    if (typeof str == 'string') {
        if (!f) { return this.escape(str, 'html');}
        if (f == 'html' || f == 'htmlall') {
            var obj = { '<': '&#60;', '>': '&#62;', '\'': '&#039;', '"': '&#034;', '&': '&#038;' };
            return str.replace(/['"<>&']/g, function (s) {return obj[s];});
        }
        if (f == 'url') {return escape(str);}
        if (f == 'urlpathinfo') {return encodeURIComponent(str).replace(/%2F/g, '/');}
        if (f == 'quotes') {return str.replace(/\'/g, '\\\'').replace(/\"/g, '\\\"');}
        if (f == 'mail') {return str.replace(/@/g, ' [AT] ').replace(/\./g, ' [DOT] ');}
        if (f == 'hex') {
            for (var i = 0, l = str.length; i < l; i++) {tmp += '%' + padnum(str.charCodeAt(i), 2, 16);} return tmp;
        }
        if (f == 'hexentity') {
            for (var i = 0, l = str.length; i < l; i++) {tmp += '&#x' + padnum(str.charCodeAt(i), 4, 16);} return tmp;
        }
        if (f == 'decentity') {for (var i = 0, l = str.length; i < l; i++) {tmp += "&#" + str.charCodeAt(i) + ";";}return tmp;}
        if (f == 'javascript') {
            var obj = { '\\': '\\\\', "'": "\\'", '"': '\\"', "\r": "\\r", "\n": "\\n", "</": "<\\/"}
            return str.replace(/[\\'"\r\n]|<\//g, function (s) {return obj[s];});
        }
    }
    return str;
},
...
range: function (a, b, step) {
    ...
    if (typeof a == 'number' && typeof b == 'number' && a < b) {
        for (var i = a; i <= b; i += step) {obj[(i - a) / step] = i;}
    }
    if (typeof a == 'string' && typeof b == 'string') {
        ...
        if (a < b) {
            for (var i = a; i <= b; i += step) {
                obj[(i - a) / step] = String.fromCharCode('' + i);
            }
        }
    }
    return obj;
...
var_dump: function (any) {
    ...
    function isOA(o) {return (isArray(o) || isObj(o)) ? true : false;}
    function isRealArr(o) {for (var p in o) {if (o.hasOwnProperty(p) && p.indexOf('__a') != 0) {return false;}}return true;}
    function pa(o) {
        ...
        if (type == 0 || type == 1) {
            ...
            for (var p in o) {if(o.hasOwnProperty(p)) {
                var to = o[p];
                __enc += (type == 0 ? '' : ('"' + p + '":')) + (isOA(to) ? pa(to) : (typeof to == 'string' ? '"' + to + '"' : to)) + ','
            }}
    ...
    if (isArray(any) || isObj(any)) {
        return pa(any).replace(/__a/g, '');
    }
    else {
        return any || '';
    }
